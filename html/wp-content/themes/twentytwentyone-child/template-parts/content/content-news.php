        ...
        <?php 
            ...
            <?php if ($numposts > 11) { ?>
                <div class="my-3 text-center">
                    <button class="btn btn-danger load-more-news">
                        Load More News
                    </button>
                </div>
			...
			<div class="no-news text-center"></div>		
...
<script>
    ...
    jQuery(function ($) {
        $('body').on('click', '.load-more-news', function () {
            ...
            'page': page,
            'security': '<?php echo wp_create_nonce("load_more_news"); ?>'
            };
            $.post(ajaxurl, data, function (response) {
            if (response != '') {
                $('.newsroom').append(response);
                var n = $( ".newsroom-item" ).length;
                $( ".count" ).text( "Displaying " + n + " recent news posts");
                page++;
            } else {
                $('.load-more-news').hide();
                $('.no-news').text( "There are no more news to display!" );
            }
            });
        });
    });
